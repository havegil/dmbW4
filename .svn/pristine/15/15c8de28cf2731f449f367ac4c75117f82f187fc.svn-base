...
import comboFarmhouse from './images/combo-croissant.png'
...
const isFarmhouse = API.loc('farmhouse_combo');
...
const getLocalOptionData = (localOptionKey = 'bfast_local_option_one', props) => {
  ...
  return {
    ...
    hideCalories: !isEggUpdate && API.net(`${localOptionKey}_6in_cal`) === null,
    calories: isEggUpdate
...
export default class BreakfastMenu extends Component {
  ...
  render() {
    ...
    return (
      <div className={`breakfastMenuFooter l-region-drop l-inverted ${isFarmhouse ? 'farmhouse-store' : ''}`}>
        ...
        <div className='c-Header_row'>
          ...
          <div className='c-Header_column bfastHeaderColumn'>
            <img src={footlongHeader}  style={{ marginLeft: -5 }} />
          </div>
        </div>
        ...
        {!allDay &&
          <img className='servedUntil11' src={servedUntil11} />
        }

        {allDay &&
          <img className='servedUntil11' src={servedAllDay} />
        }
        ...
        <div className='bfastCombo'>
          ...
          <div className='comboPrice'>${
            comboPrice ? comboPrice : "6.98"
            }</div>
          <div className='comboType'>{
            (breakfast_combo ? comboLookup[breakfast_combo] : comboLookup['coffee_juice'])
            }</div>
        </div>
