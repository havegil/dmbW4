...
export default class MyWayReward extends Component
{
  componentDidMount() {
      ...
      let timeline = new TimelineMax()
        ...
        .set(C.coin2, {top: 200, left: 0})
        .set(C.coin1, {top: -300, left: 0})

        .to(C.coin1, 4, {top: "+=300"}, 0)
        .to(C.coin2, 4, {top: "+=200"}, 0)
        .to(C.coin1, 1, {autoAlpha: 0}, 2.5)
        .to(C.coin2, 1, {autoAlpha: 0}, 2.5)

        .to(C.text1, 1, {autoAlpha: 0}, 3)

        .to(C.phone, 0.7, {top: "-=400", left: "+=300"}, 4)
        ...
        .to(C.phone, 0.7, {top: "+=328", left: "-=67"}, 11.5)
