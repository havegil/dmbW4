...
export default class PepperPromoQuad extends Component {

componentDidMount() {
    new TimelineMax({ delay: 0.5 })
       .set(this.bg, { left: 0, top: 0 })
       .set(this.bigSub, { left: 1450, top: 100 })
  ...
  render() {
    return (
      <div className='c-Promotion-2018w3-PepperPromo'>
        ...
        <div className='finalFrame' ref={(node) => this.finalFrame = node}>
          ...
          <div className='price price3' ref={(node) => this.price1 = node}>
            <p className="price">${this.props.peppercorn_price_6in} / ${this.props.peppercorn_price_12in}</p>
          </div>
        </div>
