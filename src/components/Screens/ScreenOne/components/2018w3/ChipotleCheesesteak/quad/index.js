...
export default class ChipotlePromoQuad extends Component {

componentDidMount() {
    new TimelineMax({ delay: 0.5 })
       .set(this.bg, { left: 0, top: 0 })
       .set(this.bigSub, { left: 1600, top: 100 })
  ...
  render() {
    return (
      <div className='c-Promotion-2018w3-CheesesteakPromo'>
        ...
        <div className='finalFrame' ref={(node) => this.finalFrame = node}>
          ...
          <div className='price price3' ref={(node) => this.price1 = node}>
            <p className="price">${this.props.chipotle_cheesesteak_price_6in} / ${this.props.chipotle_cheesesteak_price_12in}</p>
          </div>
        </div>
